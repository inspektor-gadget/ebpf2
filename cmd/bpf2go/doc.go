...
//    //go:generate go run code.cfops.it/sys/unimog/cmd/bpf2go foo path/to/src.c -- -I/path/to/include
...
//    //go:generate go run code.cfops.it/sys/unimog/cmd/bpf2go -cflags "$BPF_CFLAGS" foo path/to/src.c
// By exporting $BPF_CFLAGS from your build system you can then control
// all builds from a single location.
//
// For a full list of accepted options check the `-help` output.
package main
